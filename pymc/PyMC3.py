...
# MAGIC 
# MAGIC * μ corresponds to the mean of the distribution
# MAGIC     
# MAGIC * σ is the scale and corresponds roughly to the standard deviation
# MAGIC 
# MAGIC * ν is the degrees of freedom and takes values between 0 and $\infty$. The degrees of freedom corresponds to the number of independent observations minus 1. When the sample size is 8, the t-distribution used to model this would have degrees of freedom set to 7. A value of 1 corresponds to the Cauchy distribution and indicates heavy tails, while infinity corresponds to a Normal distribution. 
# MAGIC 
...
# MAGIC 
# MAGIC In this case, the mean of the distribution is 0 and the variance is given by ν/(ν - 2).
# MAGIC 
...
# MAGIC 
# MAGIC ```
# MAGIC     α_tmp = pm.Normal('α_tmp', mu=2, sd=5, shape=M)
# MAGIC     β = pm.Normal('β', mu=0, sd=10, shape=M)
# MAGIC     ϵ = pm.HalfCauchy('ϵ', 5)
# MAGIC     ν = pm.Exponential('ν', 1/30)
# MAGIC     
# MAGIC     y_pred = pm.StudentT('y_pred', mu=α_tmp[idx] + β[idx] * x_centered,
# MAGIC                          sd=ϵ, nu=ν, observed=y_m)
# MAGIC    
# MAGIC ```
# MAGIC 
# MAGIC     
